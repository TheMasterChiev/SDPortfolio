...
//Function to play a game of 2-player Tetris:
void Board::play_two(int p1, int p2, int modus, int holdings){
	...
	while (GameOver(modus, cleared) == false){
      ...
      in top row height difference*/
		...
		*this = result;
		/*If 'wissel' is > 0 then a Hold move was evaluated better and thus needs to be
		switched with the current piece:*/
...
//(Hier zou nog piece bij kunnen)
//Temporarily for clearing most lines:
int Board::evaluate_board(color c, int option){ 
	int score = 0;
	int full_row = 0;
	int blocks;
	int empties;
	...
	if(option == 1){
		//Black gets score for having filled fields:
		if(c == Black){ 
	...
	//Zorgt ervoor dat als je nog een lijn kan maken voordat het gameover is dat je dat ook doet:
					score = (score/200);
		        	}
		      	}
...
//Zorgt ervoor dat als je nog een lijn kan maken voordat het gameover is dat je dat ook doet:
  	...
  	// White gets score for having unfilled fields:
	if(c == White){
	    	//Score based on fill base rows first, give more credit to base rows:
	  	for(int i = 0; i < rows; i++){
	      		//full_row = 0;
